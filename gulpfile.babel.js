    ...
    .pipe(
      gulpMetalsmith({
        ...
        metadata: {
          projects: JSON.parse(
            fs
              .readFileSync(path.join(__dirname, "source/data/projects.json"))
              .toString()
          ),
          talks: JSON.parse(
            fs
              .readFileSync(path.join(__dirname, "source/data/talks.json"))
              .toString()
          )
        }
      })
  ...
  () => {
    ...
    gulp.watch(
      [
        ...
        path.join("source/layouts/**/*.hbs"),
        path.join("source/data/**")
      ],
      ["metalsmith", "shell"]
    );
    gulp.watch(path.join("source/scripts/**/*.js"), ["scripts"]);
    gulp.watch(path.join("source/styles/**/*.scss"), [
      "styles",
      "autoprefix",
      "csso"
    ]);
