...
gulp.task('shell', ['metalsmith'], gulpShell.task([
...
gulp.task('metalsmith', () => {
  return gulp.src(path.join('source/content/**/*.md'))
    .pipe(gulpMetalsmith({
...
gulp.task('autoprefix', ['styles'], () => {
  return gulp.src(path.join('public/assets/styles/**/*.css'))
    .pipe(gulpSourcemaps.init())
...
gulp.task('scripts', () => {
  return gulp.src(path.join('source/scripts/index.js'))
    .pipe(gulpWebpack({
...
gulp.task('images', () => {
  return gulp.src(path.join('source/images/**/*'))
    .pipe(gulpImagemin([
      gulpImagemin.gifsicle(),
      gulpImagemin.jpegtran(),
      gulpImagemin.optipng(),
      gulpImagemin.svgo({ plugins: [{ cleanupIDs: false }, { removeViewBox: false }, { minifyStyles: false }, { removeUselessDefs: false }] })
    ], { verbose: true }))
